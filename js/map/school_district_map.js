// function to draw school district map based on view selected
function draw_map(statesData) {
    ...
    info.update = function (props) {
        this._div.innerHTML = '<h4>Charter School Stats</h4>' +  (props ? 
            (props[which_year] ? '<b>' + props.name + '</b><br>' + 
                d3.format(",")(props[which_year].school) + ' Charter Schools' + '<br>' + 
                    d3.format(",")(props[which_year].student) + ' Students': '<b>' + 
                        props.name + '</b><br>Data is not available for this year.') : '');
    };
    info.addTo(map);
    ...
    // bind mouse input events on each polygon
    ...
    function showPopUp(e) {
        ...
        $('.to_district_link').click(
            function() {
                ...
                // window.location.href = baseURL + "#school_district&" + keyword2 + "&" + district_URL_name
                window.location.href = baseURL + "#school_district&Wyoming&JohnsonCountySchoolDistrict1"
                location.reload()
            }
        )
    }
