// function to draw authorizer map based on view selected
function draw_map(statesData) {
    ...
    //             ' Charter Schools' + '<br>' + d3.format(",")(props[which_year].student) + ' Students': '<b>' + 
    //                 props.name + '</b><br>Data is not available for this year.') : '<h6>Districts by this authorizer are<br>highlighted with orange border.</h6>');
    // };
    // info.addTo(map);
    ...
    // bind mouse input events to each feature
    ...
    geojson.eachLayer(function (layer) {
        ...
        for (var index in districts_served) {
             if (layer._id === districts_served[index] ) {
                layer.setStyle({
                    weight: 2,
                    color: 'orange',
                    // dashArray: '',
